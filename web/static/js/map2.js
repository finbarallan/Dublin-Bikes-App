...
async function findNearestBikeStationAndDisplayRoute(clickedLocation, stations) {
    ...
    const clickedLatLng = clickedLocation;

    const origins = [clickedLatLng];
    ...
    const distanceMatrixResponse = await getDistanceMatrix(distanceMatrixRequest);
    const durations = distanceMatrixResponse.rows[0].elements.map(element => element.duration.value);
